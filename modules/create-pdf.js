...
module.exports = function PDF(moduleOptions) {
  ...
  if (options.pdf.format) {
    ...
    if (supportedFormats.includes(format)) {
      ...
      console.error(
        ...
        " Unable to find format ('" + options.pdf.format + "')",
      );
      return;
    }
  }
  ...
  async function build(buildArgs) {
    ...
    for (let i = 0; i < routes.length; i++) {
      ...
      console.log(
        /* chalk.cyan('↻') + */
        ` Generating PDF ${i + 1}/${routes.length} at route ` + route.route,
      );
      ...
      try {
        ...
        const goToUrl = `${url.replace(/\/$/, '')}${route.route}`;
        console.log('GOTO url is ', goToUrl);
        ...
        const bytes = await page.pdf(dom);
        ...
        if (buildArgs.generated && !route.keep) {
          ...
          console.log(
            ` Removed route index file used for PDF at ${route.route}`,
          );
          await fs.rmdirSync(`./dist${route.route}`);
          console.log(
            `  Removed route directory used for PDF at ${route.route}`,
          );
        }
        await page.close();
        await browser.close();
        ...
        console.log(
          ` Failed to generated PDF ${i + 1}:${routes.length} at route ${
            route.route
          } error: ${e.message}`,
        );
      }
